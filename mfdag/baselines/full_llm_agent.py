...
from mfdag.harness.prompt_builder import FULL_AGENT_SYSTEM, build_full_agent_prompt
...
class FullLLMAgentBaseline:
    ...
    def _run_step(self, thread_row: Dict[str, str], step: Dict[str, Any]) -> None:
        ...
        for agent in self.data.agents:
            ...
            prompt = build_full_agent_prompt(profile, thread_state, self.text_max_chars)
            try:
                raw = self.llm.complete_json(
                    method="full_llm_agent",
                    system_prompt=FULL_AGENT_SYSTEM,
                    user_prompt=prompt,
                    meta={"thread_id": thread_id, "agent_id": agent["agent_id"]},
                )
                participate = bool(raw.get("participate", False))
            except Exception:
                participate = False
